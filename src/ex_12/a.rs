...
fn is_repeatable(n: &Node) -> bool {
    n.to_lowercase() != *n || n == "end"
}

//  a function that generates all paths from a given node to the end
fn traverse(node: &Node, node_map: &EdgeMap, visited: &mut HashSet<Node>) -> Vec<Path> {
    ...
    for next_node in node_map.get(node).unwrap().iter() {
        // if we can't visit the adjacent node, skip
        let cannot_visit = visited.contains(next_node) && !is_repeatable(next_node);
        if cannot_visit {
            continue;
        }
        ...
        let next_paths: Vec<Path> = next_paths.iter().map(|next_path| {
...
pub fn run() {
    ...
    let paths = traverse(&start_node, &edges_map, &mut visited);
    println!("{:?}", paths.len());
}